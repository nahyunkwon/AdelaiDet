...
# __all__ = ["fast_rcnn_inference", "NeRF_Classifier_FasterRCNN", "NeRFOutputLayers"]

# logger = logging.getLogger(__name__)

class NeRF_Classifier_FCOS(nn.Module):
    def __init__(self, nerfWeights, flagConv4feats=False, actMapDim=256, num_anchors=9):
        # NeRFWeights: 
        ...
        # self.convLayer4nerfWeightsTransform = nn.Conv2d(self.nerfFeaDim, self.actMapDim, kernel_size=3, stride=1, padding=1)
        ...
        self.classesToAnchors = nn.Conv2d(self.C_classes, self.C_classes * num_anchors, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, featMap):
        ...
        self.classifier_weights = self.classifier_weights.squeeze().squeeze()  # C x 1800
        ...
        self.featMap = torch.permute(self.featMap, (0, 2, 3, 1))  # N x H X W x C (1800)
